...
local Library = {
    ...
    Theme = {
        ...
        Accent        = Color3.fromRGB(82,  148, 246),
        AccentHover   = Color3.fromRGB(108, 170, 255),
        AccentDark    = Color3.fromRGB(28,  58,  118),
        Text          = Color3.fromRGB(225, 225, 232),
        TextDim       = Color3.fromRGB(128, 128, 145),
        TextMuted     = Color3.fromRGB(62,  62,  78),
        Toggle        = Color3.fromRGB(82,  148, 246),
        ...
        Info          = Color3.fromRGB(82,  148, 246),
    },
    ToggleKeybind = nil,
    IsMobile = false
}
...
function Library:CreateButton(parent, options)
    ...
    btnStroke.Color     = isRisky and riskyColor or self.Theme.Border
    ...
    local function spawnRipple(pos)
        local bPos  = btn.AbsolutePosition; local bSize = btn.AbsoluteSize
        local scale = self.MainScale and self.MainScale.Scale or 1
        local lx = (pos.X - bPos.X) / scale; local ly = (pos.Y - bPos.Y) / scale
        local maxD = math.sqrt(bSize.X^2 + bSize.Y^2) / scale
        local rip = Create("Frame", {
            Size = UDim2.fromOffset(0, 0), Position = UDim2.fromOffset(lx, ly),
            AnchorPoint = Vector2.new(0.5, 0.5),
            BackgroundColor3 = Color3.new(1,1,1), BackgroundTransparency = 0.84,
            BorderSizePixel = 0, ZIndex = btn.ZIndex + 1, Parent = btn,
        }, { Create("UICorner", { CornerRadius = UDim.new(1, 0) }) })
        Tween(rip, { Size = UDim2.fromOffset(maxD*2, maxD*2), BackgroundTransparency = 1 }, 0.42)
        task.delay(0.48, function() rip:Destroy() end)
    end

    btn.MouseButton1Click:Connect(function()
        local btnObj = self.Options[options.Flag]
        if btnObj and btnObj._disabled then return end
        spawnRipple(UserInputService:GetMouseLocation())
...
function Library:CreateKeybind(parent, options)
    ...
    keybindObj.SetValue = function(s, key)
        currentKey = key; s.Value = key
        keyButton.Text = ShortKey(key)
        updateKeybindClearBtn(); callback(key)
    end
...
function Library:CreateTabbox(parent, options)
    ...
    local TAB_H = 24; local HEADER_H = TAB_H + 2

    local wrapper = Create("Frame", {
        Size = UDim2.new(1, 0, 0, HEADER_H),
        BackgroundTransparency = 1, AutomaticSize = Enum.AutomaticSize.Y, Parent = parent
    })
    ...
    local tabW = math.floor(1 / math.max(#tabs, 1) * 100) / 100
    for i, tabName in ipairs(tabs) do
        local btn = Create("TextButton", {
            Size = UDim2.new(tabW, 0, 1, 0), BackgroundTransparency = 1,
            BackgroundColor3 = self.Theme.Accent, Text = "",
            AutoButtonColor = false, LayoutOrder = i, Parent = tabHeader
        }, {
            Create("UICorner", { CornerRadius = UDim.new(0, 8) }),
            Create("TextLabel", {
                Size = UDim2.new(1, 0, 1, 0), BackgroundTransparency = 1,
        ...
        local content = Create("Frame", {
            Size = UDim2.new(1, 0, 0, 0), BackgroundTransparency = 1,
            AutomaticSize = Enum.AutomaticSize.Y, Visible = false, Parent = wrapper
        }, {
            Create("UIListLayout", { SortOrder = Enum.SortOrder.LayoutOrder, Padding = UDim.new(0, 4) }),
    ...
    -- Auto-size header canvas
    task.defer(function()
        local ll = tabHeader:FindFirstChildWhichIsA("UIListLayout")
        if ll then tabHeader.CanvasSize = UDim2.new(0, ll.AbsoluteContentSize.X, 0, 0) end
    end)
...
function Library:CreateWindow(options)
    ...
    -- Window controls (close + minimise) top-right of sidebar
    local closeBtn = Create("TextButton", {
        Size = UDim2.new(0, 14, 0, 14), Position = UDim2.new(1, -28, 0, 10),
        BackgroundColor3 = self.Theme.Error, Text = "",
        AutoButtonColor = false, Parent = sideHeader
    }, { Create("UICorner", { CornerRadius = UDim.new(1, 0) }) })
    local minBtn = Create("TextButton", {
        Size = UDim2.new(0, 14, 0, 14), Position = UDim2.new(1, -28, 0, 28),
        BackgroundColor3 = self.Theme.Border, Text = "",
        AutoButtonColor = false, Parent = sideHeader
    }, { Create("UICorner", { CornerRadius = UDim.new(1, 0) }) })
    ...
    minBtn.MouseButton1Click:Connect(function()
        ...
        Tween(main, { Size = minimised and UDim2.new(0, SIDE_W, 0, 52) or UDim2.new(0, WIN_W, 0, WIN_H) }, 0.22)
        Tween(minBtn, { BackgroundColor3 = minimised and self.Theme.Warning or self.Theme.Border })
    end)
    ...
    -- Phase 1: typewriter splash, full screen, parented to gui
    local splashBg = Create("Frame", {
        Size = UDim2.new(1, 0, 1, 0), BackgroundColor3 = self.Theme.Background,
        BorderSizePixel = 0, ZIndex = 1199, Parent = gui
    })
    local splashLabel = Create("TextLabel", {
        Size = UDim2.new(1, -60, 0, 50),
        AnchorPoint = Vector2.new(0.5, 0.5),
        Position = UDim2.new(0.5, 0, 0.5, 0),
        BackgroundTransparency = 1, Text = "",
        TextColor3 = self.Theme.Text, Font = Enum.Font.GothamBold, TextSize = 32,
        TextXAlignment = Enum.TextXAlignment.Center,
        ZIndex = 1200, Parent = gui
    })

    -- Phase 2: loading card, parented to gui (not main), fully transparent to start
    local loadScreen = Create("Frame", {
        ...
        Create("UICorner", { CornerRadius = UDim.new(0, 12) }),
        Create("UIStroke", { Color = self.Theme.Accent, Thickness = 1, Transparency = 1 }),
    })
    ...
    local loadStatus = Create("TextLabel", {
        ...
        TextColor3 = self.Theme.Accent, TextTransparency = 1,
    ...
    task.spawn(function()
        -- Phase 1: typewriter
        for i = 1, #winName do
            splashLabel.Text = winName:sub(1, i)
            task.wait(0.055)
        end
        task.wait(0.9)
        Tween(splashLabel, { TextTransparency = 1 }, 0.4)
        Tween(splashBg,    { BackgroundTransparency = 1 }, 0.4)
        task.wait(0.45)
        pcall(function() splashBg:Destroy(); splashLabel:Destroy() end)

        -- Phase 2: loading card fade-in
